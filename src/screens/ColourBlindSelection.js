...
const ColourBlindSelection = ({ navigation }) => {
  const { width: screenWidth, height: screenHeight } = useWindowDimensions();
  const insets = useSafeAreaInsets();
  const [isOfflineMode, setIsOfflineMode] = useState(true);
  const [selectedMode, setSelectedMode] = useState("Green - Red (Deuteranopia)");

  const modes = [
    { id: "Green - Red (Deuteranopia)", label: "Green - Red (Deuteranopia)" },
    { id: "Red - Green (Protanopia)", label: "Red - Green (Protanopia)" },
    { id: "Blue - Yellow (Tritanopia)", label: "Blue - Yellow (Tritanopia)" },
  ];

  const selectedIndex = modes.findIndex(m => m.id === selectedMode);
  const ITEM_TOTAL_HEIGHT = 48;
  const slideAnim = useRef(new Animated.Value(selectedIndex * ITEM_TOTAL_HEIGHT)).current;
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Colour Blind Friendly</Text>
      ...
      >
        ...
        <Text style={styles.sectionLabel}>Select Colour Blind Friendly Mode</Text>
...
export default ColourBlindSelection;
